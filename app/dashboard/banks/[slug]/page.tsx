  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        logger.error(error, "Failed to load bank data from DB");
    ...
    if (!isLoading && currentBank && questions !== currentBank.questions) {
      ...
      updateQuestionBank(updatedBank)
        .then(() => {
          ...
          logger.info("Bank questions updated in DB.");
        })
        .catch(err => {
          logger.error(err, "Failed to save bank questions to DB");
  ...
  useEffect(() => {
    if (!isLoading) {
      saveAvailableTags(availableTags).catch(err => {
        logger.error(err, "Failed to save tags to DB");
